...
function Post(props) {
    ...
    React.useEffect(()=>{
        ...
        .then((data)=>{
            if(data.status)
                props.setCommentList([...data.data]);
            else{
    ...
    return (
        <StyledPost key = {props.post.user_name} >
            ...
            <form className={classes.root} method = "post" noValidate autoComplete="off" onSubmit = {props.handleCommentSubmit}>
                <TextareaAutosize
                    style = {{display : "block",width : "100%",height : "15vh", marginTop : "2em"}}
        ...
        <CardContent  >
        {renderComments(props.commentList)}
        </CardContent>
        </StyledPost>
    )
}
